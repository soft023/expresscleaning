...
function connect(service, normalize) {
  ...
  const separator = (0, import_date_picker2.getLocaleSeparator)(locale);
  ...
  function getYearTableCellState(props) {
    ...
    const isOutsideRange = (0, import_utils.isValueWithinRange)(value, min?.year, max?.year);
    const cellState = {
      ...
      inRange: isRangePicker && ((0, import_date_picker2.isDateWithinRange)(dateValue, selectedValue) || (0, import_date_picker2.isDateWithinRange)(dateValue, hoveredRangeValue)),
      value: dateValue,
      get disabled() {
  ...
  function getDayTableCellState(props) {
    ...
    const isFirstInSelectedRange = isRangePicker && (0, import_date_utils.isDateEqual)(value, selectedValue[0]);
    const isLastInSelectedRange = isRangePicker && (0, import_date_utils.isDateEqual)(value, selectedValue[1]);
    ...
    const isFirstInHoveredRange = hasHoveredRange && (0, import_date_utils.isDateEqual)(value, hoveredRangeValue[0]);
    const isLastInHoveredRange = hasHoveredRange && (0, import_date_utils.isDateEqual)(value, hoveredRangeValue[1]);
    const isSelected = selectedValue.some((date) => (0, import_date_utils.isDateEqual)(value, date));
    const cellState = {
      ...
      get focused() {
        return (0, import_date_utils.isDateEqual)(value, focusedValue) && (!cellState.outsideRange || outsideDaySelectable);
      },
      get ariaLabel() {
  ...
  return {
    ...
    getYearTableCellProps(props) {
      ...
      return normalize.element({
        ...
        colSpan: columns,
        role: "gridcell",
        "aria-selected": (0, import_dom_query.ariaAttr)(cellState.selected),
        "data-selected": (0, import_dom_query.dataAttr)(cellState.selected),
        "aria-disabled": (0, import_dom_query.ariaAttr)(!cellState.selectable),
        "data-value": value
      });
    ...
    getInputProps(props = {}) {
      ...
      return normalize.input({
        ...
        onBeforeInput(event) {
          ...
          if (!(0, import_date_picker2.isValidCharacter)(data, separator)) {
            event.preventDefault();
          }
        },
        onClick(event) {
        ...
        onInput(event) {
          ...
          send({ type: "INPUT.CHANGE", value: (0, import_date_picker2.ensureValidCharacters)(value, separator), index });
        }
      });
    },
    getMonthSelectProps() {
