...
var tags_input_machine_exports = {};
__export(tags_input_machine_exports, {
  machine: () => machine
});
module.exports = __toCommonJS(tags_input_machine_exports);
...
var dom = __toESM(require("./tags-input.dom.js"));
var { and, not, or } = (0, import_core.createGuards)();
var machine = (0, import_core.createMachine)({
  ...
  implementations: {
    guards: {
      ...
      isCaretAtStart: ({ scope }) => (0, import_dom_query.isCaretAtStart)(dom.getInputEl(scope))
    },
    effects: {
      trackInteractOutside({ scope, prop, send }) {
        return (0, import_interact_outside.trackInteractOutside)(dom.getInputEl(scope), {
          exclude(target) {
            const itemEls = dom.getItemEls(scope);
            return itemEls.some((el) => (0, import_dom_query.contains)(el, target));
          },
      ...
      autoResize({ context, prop, scope }) {
        ...
        queueMicrotask(() => {
          ...
          fn_cleanup = (0, import_auto_resize.autoResizeInput)(inputEl);
        });
        return () => {
          fn_cleanup?.();
        };
    ...
    actions: {
      ...
      deleteTag({ context, scope, event, refs }) {
        ...
        context.set("value", (prev) => (0, import_utils.removeAt)(prev, index));
      },
      ...
      syncInputValue({ context, scope }) {
        ...
        if (!inputEl) return;
        (0, import_dom_query.setElementValue)(inputEl, context.get("inputValue"));
      },
      syncEditedTagInputValue({ context, event, scope }) {
        ...
        (0, import_dom_query.setElementValue)(editTagInputEl, context.get("editedTagValue"));
      },
      addTag({ context, event, computed, prop, refs }) {
        ...
        if (guard) {
          const nextValue = (0, import_utils.uniq)(value.concat(inputValue));
      ...
      addTagFromPaste({ context, computed, prop, refs }) {
        (0, import_dom_query.raf)(() => {
          ...
          if (guard) {
            ...
            const nextValue = (0, import_utils.uniq)(value.concat(...trimmedValue));
            context.set("value", nextValue);
            const prevLog = refs.get("log");
            refs.set("log", {
              prev: prevLog.current,
